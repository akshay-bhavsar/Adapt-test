  ...
  // Private functions
    ...
    _checkAssessmentsComplete: function() {
      ...
      for (var id in states) {
        var state = states[id];
        if (!state.includeInTotalScore || !state.isAvailable) continue;
        if (!state.isComplete) {
    ...
    _setPageProgress: function() {
      ...
      for (var k in this._assessments._byPageId) {
        ...
        for (var i = 0, l = assessments.length; i < l; i++) {
          ...
          if (assessmentState.includeInTotalScore && !assessmentState.isPass && !assessmentState.isAvailable) continue;
